...
class SegTracker():
    ...
    def __init__(self, segtracker_args, sam_args, aot_args) -> None:
        """
         Initialize SAM and AOT. 初始化sam和aot
        """
    ...
    def seg(self, frame):
        '''
        Arguments:参数
            frame帧: numpy array (h,w,3)
        Return:返回值
            origin_merged_mask原始_合并后的_遮罩: numpy array (h,w)
        '''
        frame = frame[:, :, ::-1]
        anns = self.sam.everything_generator.generate(frame)

        # anns is a list recording all predictions in an image
        # anns是一个列表，记录了图像中的所有预测
        if len(anns) == 0:
            return
        # merge all predictions into one mask (h,w) 合并所有预测到一个掩码(h,w)
        # note that the merged mask may lost some objects due to the overlapping  注意合并后的蒙版可能会因为重叠而丢失一些对象
    ...
    def add_mask(self, interactive_mask: np.ndarray):
        '''
        Merge interactive mask with self.origin_merged_mask 使用self.origin_merged_mask生成交互式掩码
        Parameters:
            interactive_mask: numpy array (h, w) 交互式掩码
        Return:
            refined_merged_mask: numpy array (h, w) 精致合并掩码
        '''
    ...
    def detect_and_seg(self, origin_frame: np.ndarray, grounding_caption, box_threshold, text_threshold,
                       box_size_threshold=1, reset_image=False):
        '''
        Using Grounding-DINO to detect object acc Text-prompts 使用G-D去检测物体，根据提供的文本
        Retrun:
            refined_merged_mask: numpy array (h, w)
        ...
        for i in range(len(boxes)):
            ...
            if (bbox[1][0] - bbox[0][0]) * (bbox[1][1] - bbox[0][1]) > annotated_frame.shape[0] * annotated_frame.shape[
